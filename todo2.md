...
| A19 🔴 | **A domborzati zaj nem mozog a lemezzel (felhasználói elvárás, 2026-09-22)** | **ELLENŐRIZVE, a kód megerősíti.** Deep-time-ban CSAK a lemez-magok forognak el (`PlateMotion.MovedSeeds`); az elsődleges zaj, a `MountainMask` és a másodlagos részletzaj a **rögzített világ-pozícióból** számolódik (`CrustElevation.ComputeNoiseBasis(worldSeed, x, y, z)` — `DeepTimeErosionGlaciation.cs:55`, `PlateBoundaryEffect.cs:201`), és ez az ND-63-ban szándékként is rögzítve van („kizárólag a world seedtől és a pozíciótól függenek"). Következmény: a lemez alatt **átcsúszik** a domborzat-textúra — a hegyvonulat helyben marad, miközben a lemez elvándorol alóla. **Elvárás:** a zaj a lemez saját vonatkoztatási rendszerében értékelődjön ki, azaz a mintavételi pontot előbb a lemez Euler-pólusa körül `R(−ωt)`-vel vissza kell forgatni, és a zaj így egyenletesen, arányosan vándorol a lemezzel. **Amit tudni kell hozzá:** (a) lemezhatáron a szomszédos tile-ok MÁS keretben vannak → a jelenlegi kétlemezes `BlendedBaseElevationFromNoiseBasis` keverés miatt **két** zaj-kiértékelés kell pontonként (kb. kétszeres zajköltség), különben varrat lesz a határon; (b) az ND-122/ND-131 terrain-bázis lemez-gyorsítótár ma **pozíció-kulcsú és időfüggetlen** — ezzel időfüggővé válik, a cache-stratégiát újra kell gondolni (ez a hideg Build 2,3 s-os tétele volt); (c) `t=0`-nál a forgatás egzakt identitás, tehát a **statikus világok bitre változatlanok**, csak a `t>0` deep-time kimenet módosul → ND-108 generátorverzió-emelés kell. | ND-63, ND-122/131, ND-108; ld. A20, C7 |
| A20 🟠 | **Deep-time erózió: ma csak uplift-relaxáció, nem erózió (felhasználói észrevétel, 2026-09-22)** | **ELLENŐRIZVE.** A `DeepTimeErosionGlaciation` teljes „eróziója" egyetlen zárt alakú exponenciális relaxáció, ami a **lemezhatár-uplift bónuszt** húzza le a 35%-ára (`UpliftRelaxationElevation`, τ=50 Myr) — az alap-eleváció (elsődleges/másodlagos zaj) **időben teljesen változatlan**. Nincs vízhajtotta bevágódás, nincs hordalékszállítás, nincs lerakódás, nincs medencefeltöltés, és nincs összekötve a MEGLÉVŐ hidrológiával (folyóhálózat, vízgyűjtők, tavak — ND-124/ND-127), pedig az az adat rendelkezésre áll. A parti abrázió és a jégtakaró-eróziós tag szintén hiányzik (a `GlaciationPeriodMyr`/`AmplitudeK` ma csak a jégvonalat mozgatja, a felszínt nem koptatja). **Elvárás:** a deep-time csúszka legyen érezhető a domborzaton is, ne csak a hegycsúcsok lelapulásában. **Figyelem az I1-re:** egy iteratív eróziós integrátor megtörné az ND-04 timestep-invarianciát (a mai relaxáció azért zárt alakú, hogy `t` közvetlen lekérdezése = lépésenként szimulálva). Vagy zárt alakú marad, vagy rögzített, `t`-től független lépésszámra kell kötni — ez ND-döntést igényel. Plusz: a modul ma nyers `Math.Exp`/`Math.Sin`-t használ, tehát **ma sem bit-determinisztikus platformok között** (a `DeterministicMath`-ra váltás itt esedékes). | ND-44, ND-04, ND-23, ND-124 |
...
6. **Tektonika-blokk (2026-09-22, felhasználói kérés):** a javasolt sorrend
   **A19 → A20 → C7 → C8 → C9**. Indok: az A19 (lemezzel együtt mozgó zaj) és
   az A20 (valódi erózió) a `t>0` kimenetet módosítja, de a `t=0` világot
   bitre változatlanul hagyja, tehát egy ND-108 verzióemeléssel elintézhetők;
   a C7–C9 ezzel szemben a lemez-reprezentációt cseréli le, és a C5/C6-tal
   közös tervet kíván. Egy verzióemelésbe érdemes összefogni az A19-et és az
   A20-at, különben kétszer törik a seed-kompatibilitás.
