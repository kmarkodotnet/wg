...
| 2 | **Klíma: a biome-osztályozó nem látja a csapadékot** | ~~Konstans-kalibráció.~~ → **NEM konstans-probléma** (2026-09-21, ND-126): a `BiomeClassification.Classify(temperatureK, isOceanic)` szignatúrája a magyarázat — a csapadék nem bemenet, ezért négy tisztán hőmérsékleti osztály van, és abból **tökéletes szélességi sávok** lesznek (a >20 °C sáv render-színe homoksárga — ezt olvastad sivatagnak). A csapadék-mező viszont jó: egy hőmérsékleti sávon belül a szórása **52–140×** (P10→P90), vagyis ugyanazon a szélességen már most van száraz és nedves szárazföld. **Döntést kér: ND-126 (A) vagy (B).** Mellette két MÉRT konstans-hiba (ND-126b): a termikus szél a sarkoknál 120–126 m/s (Egyenlítőn 7,4), és a sark–egyenlítő hőmérsékleti esés túl meredek (−62 °C). | ND-126, ND-41 |
...
| 3 | **Klíma: (A) vagy (B)?** | Nincs mit tesztelni — az ND-126 két opcióját olvasod el. (A) Whittaker-jellegű 2D osztályozás (hőmérséklet × csapadék, új biome-értékekkel), (B) minimális aridity-módosító (csak sivatag jön létre újként). | A biome-mező fogyasztói a régiónevek, a talaj-termékenység, a panelek és a render-színek — mindegyik érintett. Verzió-emelés kell (a világ biome-mezője és a régiónevek megváltoznak), de a domborzat NEM. |
