...
public class ImpactFieldIntegrationTests
{
    ...
    [Fact]
    public void ApplyToFieldWithPrecomputedCratersMatchesSeedTimeOverload()
    {
        // A viewer a krater-listat MASHOZ is hasznalja, ezert a
        // (field, craters) overloadot hivja - ez BITRE ugyanazt kell adja,
        // mint a (field, seed, time) valtozat (a viewer-oldali mezo-hatas
        // konzisztenciaja a Core-lanccal, ND kraterek).
        var field = new Dictionary<TileId, double>();
        for (uint u = 0; u < 8; u++)
            for (uint v = 0; v < 8; v++)
                field[TileId.FromFaceLevelUV(0, 5, u + 8, v + 8)] = 100.0 * (u + v);

        const double timeMyr = 400.0;
        List<ImpactCratering.CraterRecord> craters = ImpactCratering.GenerateCratersUpToTime(WorldSeed, timeMyr);

        Dictionary<TileId, double> viaSeedTime = ImpactCratering.ApplyToField(field, WorldSeed, timeMyr);
        Dictionary<TileId, double> viaCraters = ImpactCratering.ApplyToField(field, craters);

        Assert.Equal(viaSeedTime.Count, viaCraters.Count);
        foreach (KeyValuePair<TileId, double> kv in viaSeedTime)
            Assert.Equal(kv.Value, viaCraters[kv.Key]);
    }

    [Fact]
    public void ApplyToFieldWithEmptyCratersReturnsFieldUnchanged()
    {
        var field = new Dictionary<TileId, double> { [TileId.FromFaceLevelUV(0, 3, 1, 1)] = 42.0 };
        Assert.Same(field, ImpactCratering.ApplyToField(field, new List<ImpactCratering.CraterRecord>()));
        List<ImpactCratering.CraterRecord> nullCraters = null!;
        Assert.Same(field, ImpactCratering.ApplyToField(field, nullCraters));
    }
